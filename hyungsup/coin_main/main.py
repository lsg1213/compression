...
parser.add_argument("-ni", "--num_iters", help="Number of iterations to train for", type=int, default=10)  #원래 50000이었음
...
for i in range(min_id, max_id + 1):
    ...
    # Load image
    img = imageio.v2.imread(f"kodak-dataset/kodim{str(i).zfill(2)}.png")
    ...
    # Load audio
    audio = 'audio/0_1.wav'
    y, sr = torchaudio.load(audio)  # 길이 설정 어떻게 하지?
    print("y shape: ", y.shape)
    #y, sr = librosa.load(audio, mono=True, duration=10)
    ...
    coordinates, features = util.to_coordinates_and_features(melspec)
    ...
    print(f'Model size: {model_size:.1f}kB')       # model size (kB)
    fp_bpp = util.bpp(model=func_rep, image=melspec)
    print(f'Full precision bpp: {fp_bpp:.2f}')     # full precision bpp
    ...
    print("\n------start evaluation------")
    func_rep.eval()
    with torch.no_grad():  # model evaluation
        ...
        #audio_recon = func_rep(coordinates).reshape(melspec.shape[1], melspec.shape[2], 1).permute(2, 0, 1)
        audio_recon = func_rep(coordinates)
    print("after eval audio_recon shape: ", audio_recon.shape)

    # melspec shape로 reshape ---> 이렇게 하는게 맞나??
    recon_melspec = np.reshape(audio_recon, (melspec.shape[0], melspec.shape[1], melspec.shape[2]))
    #recon_melspec = np.reshape(audio_recon, (melspec.shape[2], melspec.shape[1], melspec.shape[0]))
    print("recon_melspec shape: ", recon_melspec.shape)
    print(recon_melspec)
    #recon_melspec = torchaudio.functional.DB_to_amplitude(recon_melspec, ref=1, power=0.5)
    transform = torchaudio.transforms.InverseMelScale(512, sample_rate=sr)(recon_melspec)
    y = torchaudio.transforms.GriffinLim(n_fft=512)(transform[:, :257])

    #wav = transform(recon_melspec)
    #wav = torch.istft(wav, n_fft=512)
    #print("wav shape: ", wav.shape)
    #soundfile.write("reconstruction.wav", wav, samplerate=sr)

    # recon_melspec = np.array(recon_melspec, dtype=np.float32)
    # recon_melspec = librosa.db_to_amplitude(recon_melspec)
    # print(recon_melspec)
    # S = librosa.feature.inverse.mel_to_audio(recon_melspec)
    # print("S shape: ", S.shape)
    # y = librosa.griffinlim(S)
    #
    soundfile.write("C:/Users/USER/Desktop/10.wav", y, samplerate=sr)
